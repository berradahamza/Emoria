...
export const useJournalStore = defineStore("journal", {
  ...
  getters: {
    ...
    /**
     * Toutes les réussites de l'historique, regroupées par date (du plus récent au plus ancien).
     * Chaque élément : { date: 'YYYY-MM-DD', successes: [{ text, tag }] }
     */
    allSuccessesGroupedByDate() {
      const _ = this.updateCounter;
      return Object.keys(this.savedEntries)
        .sort((a, b) => b.localeCompare(a)) // plus récent en premier
        .filter((dateKey) => {
          const entry = this.savedEntries[dateKey];
          return Array.isArray(entry?.successList) && entry.successList.length > 0;
        })
        .map((dateKey) => ({
          date: dateKey,
          successes: this.savedEntries[dateKey].successList,
        }));
    },

    /** Tous les tags uniques utilisés dans l'historique */
    allUsedTags() {
      const _ = this.updateCounter;
      const tagSet = new Set();
      for (const entry of Object.values(this.savedEntries)) {
        if (!Array.isArray(entry?.successList)) continue;
        for (const item of entry.successList) {
